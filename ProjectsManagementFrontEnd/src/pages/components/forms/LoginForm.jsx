...
const LoginForm = () => {
    ...
    const onSubmit = async (data) => {
        try {
          ...
          console.log( 'Authentication With success! -> ', data.username );
        } catch (error) {
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="max-w-md mx-auto mt-8 bg-white p-8 rounded-xl shadow-lg">
            ...
            <label className='block text-gray-700 text-sm font-bold mt-3 mb-2'>Username</label>
                    <input className=' appearance-none outline-none bg-transparent w-full py-2 px-3 textgray-700 leading-tight border-b border-gray-400' type="username"  placeholder='username or email'
            ...
            <div className="mt-4">
                ...
                <p className="mt-2 text-sm">
                    Don't have an account? <Link to="/register" className="text-blue-500">Create one here</Link>.
                </p>
            </div>
        </form>
