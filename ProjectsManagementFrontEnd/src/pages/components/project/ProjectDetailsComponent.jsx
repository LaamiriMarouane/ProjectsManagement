...
const ProjectDetails = ({ metadata }) => {
  const { projectLongName, projectName, user, type, theme, status, creatingtime, description, public: isPublic } = metadata[0];
    ...
    <div className="flex items-start justify-center mx-auto">
      <div className="max-w-[1024px]">
        <div className="flex items-center justify-between border-b border-b-slate-400 pb-2">
          <div className='flex items-center justify-center gap-3'>
            <h2 className="text-xl font-bold mb-1">{projectLongName}</h2>
            <span className={`bg-${isPublic ? 'purple-800' : 'gray-400'} text-white text-sm font-semibold px-2.5 py-0.5 rounded-full`}>
              {isPublic ? 'public' : 'private'}
            </span>
          </div>
          <h2 className="text-lg font-semibold font-mono mb-1">{projectName}</h2>
        </div>
        ...
        <div className="mt-3 text-base font-semibold">
          ...
          <span className={`text-xs font-bold mx-1 px-1 py-1.5 rounded shadow uppercase ${ status ? ' bg-green-400' : 'bg-red-400' } `}>{status ? 'enabled' : 'disabled'}</span>
        </div>
