...
import { FaLink, FaLongArrowAltLeft, FaLongArrowAltRight } from 'react-icons/fa';
import Filter from '../../utils/Filter';
import { Link } from 'react-router-dom';

const HomePage = () => {

    const listOfPublicProjects = useSelector((state) => state.demand.Demands);
    const [filterType, setFilterType] = useState('');
    const [currentPage, setCurrentPage] = useState(1);
    const itemsPerPage = 2;       

        const projectTypes = Array.from(new Set(listOfPublicProjects.map((project) => project.type)));
        const projectThemes = Array.from(new Set(listOfPublicProjects.map((project) => project.theme)));

        const filteredProjects = filterType
            ? listOfPublicProjects.filter((project) => project.type === filterType)
            : listOfPublicProjects;
            ...
            <div className="mb-8 w-full">
                <div className="space-y-3">
                    {currentProjects.map((project) => (
                        <div key={project.id} className="bg-white px-4 py-2 rounded shadow">
                            ...
                            <p className="text-gray-600 mb-1 py-2 w-full border-t border-b border-slate-100">{project.description}</p>
                            ...
                            <div className="flex items-center justify-between mt-3 text-sm font-semibold">
                                <Link to = { '/details' } className="text-black underline flex items-center hover:text-blue-700">
                ...
                <div className="flex justify-center gap-4 mt-4 items-center">
                    ...
                    <span className="mx-4 text-sm font-bold text-black">
                        <span className='py-2 px-3.5 bg-slate-400 rounded-full' >{currentPage}</span>  / {totalPages}
                    </span>
                    <button
                        onClick={() => handlePageChange(currentPage + 1)}
